...
import subprocess #JANEK for opening pdf file
import sys
    ...
    class DisplayedWindow(QDialog):
        ...
        def __init__(self):
           ...
           #JANEK type points button
            ...
            #JANEK INFO button
            self.btn_info = QtGui.QToolButton(self.window_plugin)
            self.btn_info.setToolTip('About J Rectifier')
            self.btn_info.setIcon( QIcon(os.path.join(os.path.dirname(__file__),'icons/info.png') ))
            self.btn_info.setIconSize( QSize(25, 25) )
            self.btn_info.clicked.connect(lambda: self.click_btn_info())
            self.btn_info.setStyleSheet('border: none;')
            self.zoom_layout.addWidget(self.btn_info, 0, 5, Qt.AlignCenter)
...
#JANEK==============BUTTON FUNCTIONS====================
        def click_btn_open_raster(self): 
            ...
            if qgis.utils.iface.mapCanvas().mapRenderer().destinationCrs().authid()[5:] == '':
                ...
                if raster_path != '': #if somebody didnt abandoned opening file
                    ...
                    temp_path = os.path.join(os.path.dirname(__file__),'temp_files')
        ...
        def click_btn_exec(self):
            ...
            if raster_path is not None:    
                ...
                if len(points_xyXY) >= min_points:
                    ...
                    exec_dial.setWindowTitle('Rectification summary')
                    ...
                    pdf_checkbox.setText('create PDF report')
                    ...
                    def make_pdf_rap(outfile):
                        ...
                        #paths
                        pdf_path = outfile[:-len(outfile.split('/')[-1].split('.')[-1])] + '_report.pdf' 
                        ...
                        html_view.printer.setOutputFileName(outfile[:-4] + '_report.pdf') #pdf raport path                   
                        ...
                        try:
                            ...
                            def click_ok_button(): # georeferencing the file with GDAL
                                ...
                                if outfile_path != '':
                                    ...
                                    if os.path.isfile(outfile_path): #if the file with the same name allready exist - need to delete it and remove from qgis, if not, there can come some problems with overwritting
                                        #Removing layer from qgis
                                        layermap = QgsMapLayerRegistry.instance().mapLayers()
                                        RemoveLayers = []
                                        for name, layer in layermap.iteritems():
                                            if layer.isValid():
                                                if layer.source() == outfile_path:
                                                    RemoveLayers.append(layer.id())
                                        if len(RemoveLayers) > 0:
                                            QgsMapLayerRegistry.instance().removeMapLayers( RemoveLayers )
        ...
        def click_btn_info(self):
            subprocess.Popen([os.path.join(os.path.dirname(__file__),'Documentation.pdf')], shell=True)
