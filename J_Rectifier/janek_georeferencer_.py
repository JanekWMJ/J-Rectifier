...
class JanekGeoreferencer:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'JanekGeoreferencer_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Janek Georeferencer')
        # TODO: We are going to let the user set this up in a future iteration
        self.toolbar = self.iface.addToolBar(u'JanekGeoreferencer')
        self.toolbar.setObjectName(u'JanekGeoreferencer')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('JanekGeoreferencer', message)
    ...
    def initGui(self):
        ...
        self.action = QAction(
            QIcon(':/plugins/JanekGeoreferencer/icon.png'),
            'Janek Georeferencer', self.iface.mainWindow())
        # connect the action to the run method
        self.action.triggered.connect(self.run)

        # Add toolbar button and menu item
        self.iface.addToolBarIcon(self.action)
        self.iface.addPluginToRasterMenu('&Janek Georeferencer', self.action)

    def unload(self):
        """Removes the plugin menu item and icon from QGIS GUI."""
        self.iface.removePluginRasterMenu('&Janek Georeferencer', self.action)
    ...
    class DisplayedWindow(QDialog):
        ...
        def __init__(self):
            ...
            self.window_plugin.setWindowTitle('JANEK Georeferencer')
            ...
            self.btn_del_point.setToolTip("Delete selected rows\n[CTRL+D]")
           ...
           #JANEK type points button
            ...
            '''def CloseEvent(self, event):
                self.window_plugin = self
                global point_plug_path, point_qgis_path, err_qgis_path, err_plug_path
                #Chancing pointTool for normal one
                quit_msg = "Are you sure you want to exit the program?"
                reply = QtGui.QMessageBox.question(None, 'Message', quit_msg, QtGui.QMessageBox.Yes, QtGui.QMessageBox.No)
        ...
        def click_btn_open_raster(self): 
            ...
            if qgis.utils.iface.mapCanvas().mapRenderer().destinationCrs().authid()[5:] == '':
                ...
                if point_plug_path != None and point_qgis_path != None and err_qgis_path != None:
                    ...
                    for layer in QgsMapLayerRegistry.instance().mapLayersByName('Janek Georeferencer: points'):
                        if layer.source() == point_qgis_path:
                            QgsMapLayerRegistry.instance().removeMapLayer(layer)
                    for layer in QgsMapLayerRegistry.instance().mapLayersByName('Janek Georeferencer: errors'):
                ...
                if raster_path != None:
                    raster_path = QFileDialog.getOpenFileName(None,"Select raster file",raster_path[:-len(raster_path.split('/')[-1])],"") #Getting the file (from raster catalog)
                    if raster_path != '':
                        path_memory = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'path\\path_memory.txt'), 'w') #wrighting a new path for future
                        path_memory.write(raster_path)
                        path_memory.close()
                else:
                    try:
                        path_memory = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'path\\path_memory.txt'), 'r') #opening textfile with path
                        open_path = path_memory.readline()
                        path_memory.close()
                        raster_path = QFileDialog.getOpenFileName(None,"Select points file", open_path, "") #wrighting a new path for future
                        if raster_path != '':
                            path_memory = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'path\\path_memory.txt'), 'w')
                            path_memory.write(raster_path)
                            path_memory.close()
                    except:
                        raster_path = QFileDialog.getOpenFileName(self.window_plugin,"Select raster file","","") #GLOBAL
                        if raster_path != '':
                            path_memory = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'path\\path_memory.txt'), 'w') #wrighting a new path for future
                            path_memory.write(raster_path.encode('utf8'))
                            path_memory.close()
                
                if raster_path != '': #if somebody didnt abandoned opening file
                
                    '''if sys.platform == 'win32':
                        raster_path = raster_path.encode('cp1250') # Decoding Windows  coding format - cp1250'''
                                        
                    lyr = QgsRasterLayer(raster_path, 'JANEK Georeference raster')
                    ...
                    qgis.utils.iface.addVectorLayer(err_qgis_path, 'Janek Georeferencer: errors', 'ogr')
                    qgis.utils.iface.addVectorLayer(point_qgis_path, 'Janek Georeferencer: points', 'ogr')
        ...
        def click_btn_exec(self):
            ...
            if raster_path is not None:    
                ...
                if len(points_xyXY) >= min_points:
                    ...
                    exec_dial.setWindowTitle('Georeference summary')
                    ...
                    if tr_method == 'order 1' or tr_method == 'order 2' or tr_method == 'order 3' or tr_method == 'tps': # for POLYNOMIAL 123 and TPS methods
                        ...
                        def click_ok_button(): # georeferencing the file with GDAL
                            ...
                            if outfile_path != '':
                                ...
                                qgis.utils.iface.addRasterLayer(outfile_path, outfile_path.split('/')[-1][:-4])
                        ...
                        def click_ok_button(): # georeferencing the file with GDAL
                            ...
                            progdialog = QtGui.QProgressDialog("Georeferencing", None, 0, 100, None)
                            #progdialog.setWindowFlags(Qt.WindowStaysOnTopHint)
                            progdialog.setWindowTitle('JANEK Georeferencer')
...
#JANEK if PointTool works - mouse clicking deliver new points to GCP table, if PointTool2 works - it doesn't deliver them
        ...
        class PointTool(QgsMapTool): #JANEK pointtool that makes mouse clicking adding GCP points
            ...
            def canvasReleaseEvent(self, event):
                ...
                for i in range(self.table.rowCount()):
                    if self.table.item(i,1).text() == '-' and self.table.item(i,2).text() == '-' and table_full == True:
                ...
                if self.zoom_checkbox.isChecked(): # works only when user wants it
                    ...
                    if len(gcp_table) > 1 and self.table.item(current_id - 1,3).text() == '-' and self.table.item(current_id - 1,4).text() == '-': #zooming #1 
                        ...
                        if self.table.item(current_id, 3) is not None and self.table.item(current_id, 4) is not None: # if next XY exist 
                            if self.table.item(current_id, 3).text() != None and self.table.item(current_id, 4).text() != None: # and of course is not empty
        ...
        class PointTool_MainQgisCanvas(QgsMapTool): #JANEK pointtool that makes mouse clicking adding GCP points
            ...
            def canvasReleaseEvent(self, event):
                ...
                #JANEK ======== Passing X Y values to table
                for i in range(self.table.rowCount()):
                    if self.table.item(i,3).text() == '-' and self.table.item(i,4).text() == '-' and table_full == True:
                ...
                if self.zoom_checkbox.isChecked(): # works only when user wants it
                    ...
                    if len(gcp_table) > 1 and self.table.item(current_id - 1,1).text() == '-' and self.table.item(current_id - 1,2).text() == '-': #zooming if 
                        ...
                        if self.table.item(current_id, 1) is not None and self.table.item(current_id, 2) is not None: # if next xy exist 
                            if self.table.item(current_id, 1).text() != None and self.table.item(current_id, 2).text() != None: # and of course is not empty
